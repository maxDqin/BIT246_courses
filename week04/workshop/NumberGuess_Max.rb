...
class Game
  ...
  def display_instructions
    ...
    puts "This game randomly generates a number from 1 to 1000 and" # Task 2 - 2/2
  ...
  def generate_number
    ...
    return randomNo = 1 + rand(1000) # Task 2 - 1/2
  ...
  def play_game
    ...
    loop do
      ...
      reply = STDIN.gets #Collect the player's answe
      $noOfGuesses = $noOfGuesses + 1 # Task 1 - 3/4
  ...
  def display_credits
    ...
    if $gameCount != 0 then
      ...
      puts "You have played #{$gameCount} time(s), and the average number of guesses per game is #{$avgNoOfGuesses}"
    else
      puts "You have played #{$gameCount} time(s), and the average number of guesses per game is 0"
    end
